...
>
> ⚠️ 本文档与「内容发布流程与质量规范」中的 Agent A/B 流水线是不同维度的概念：
...
**示例**：
- 内容发布发布 Agent → 任务索引 + 内容发布相关规范；仅在改 KB 时加载协作规范
- 研究资料整理 Agent → 任务索引 + 用户自建 `知识/<主题>/` 相关文件
...
| 字段 | 说明 | 示例 |
|------|------|------|
| `[agent-标识]` | 谁做的修改 | `[示例-AgentA]`, `[大钱-主控]`, `[deepseek-chat]` |
...
```bash
git add .
git commit -m "[示例-AgentA] 修复平台API手册 freepublish 说明 + 扩展面试tags — 平台API开发手册.md, 面试经验.md"
git push origin main
```

### 命名约定

- 用 agent 标识或模型名（如 `示例-AgentA`, `deepseek-chat`, `qwen`）
- 固定使用一个名称，方便追溯
...
```
[示例-AgentA] 正在编辑GPU分享.md - 预计30分钟完成
```
...
41e8d34 [示例-AgentA] 补充内容运营策略文档 — 内容运营策略.md
===== 当前 HEAD: 1559c08 =====
```
