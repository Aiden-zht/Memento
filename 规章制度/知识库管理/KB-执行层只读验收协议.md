...
version: 1
changelog: "[agent] 创建：统一规则变更后的执行层只读验收输出格式"
---
...
1. **KB 入口一致性**
   - AGENTS.md、README.md、index.md、任务能力索引、强制规则索引、其他入口索引是否仍引用旧目录名、旧规则口径、已废弃的权威源。
2. **skill/cron 依赖声明**
...
相关：[[规章制度/知识库管理/知识库内容治理规范/07-Agent行为约束]] · [[规章制度/知识库管理/任务类型索引]]
